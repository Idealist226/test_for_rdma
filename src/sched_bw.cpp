...
#define LATENCY_SIZE    2
// #define BANDWITH_SIZE   524288
#define BANDWITH_SIZE   65536
...
static void usage(const char *argv0)
{
	...
	printf("  -t, --lat-iters=<iters>   number of changes for latency-sensitive applications (default 100000)\n");
	printf("  -b, --bw-iters=<iters>    number of changes for bandwidth-sensitive applications (default 1000)\n");
...
int test_time(struct context *ctx, int iters, int *routs)
{
	...
	while (size <= max_size) {
		...
		if (servername) {
			if (post_send_poll(ctx, iters, ctx->app_type==LATENCY?size:max_size) < 0) {
		...
		if (ctx->app_type == LATENCY) {
			...
			lat[i][1] = (end.tv_sec - start.tv_sec) * 1000000 +
				(end.tv_usec - start.tv_usec);
			long long bytes = (long long) max_size * iters;
...
void print_time() {
	...
	printf("%-20s : %d\n", "Priority", priority);
	printf("=======================================================================================\n");
	printf("%-20s %-20s %-20s %-20s %-20s %-20s %-20s %-20s\n", "#lat_iters", "#bytes[Lat App]", "BW[Gbps]", "Lat/iters[us]",
		"#bw_iters", "#bytes[Bw App]", "BW[Gbps]", "Lat/iters[us]");

	while (size <= max_size) {
		printf("%-20d %-20d %-20.3lf %-20.2f %-20d %-20d %-20.3lf %-20.2f\n", 
			lat_iters, size, bw[i][0], lat[i][0]/lat_iters, bw_iters, max_size, bw[i][1], lat[i][1]/bw_iters);
